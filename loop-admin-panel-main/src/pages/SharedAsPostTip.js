...
function SharedAsPostTip(props) {
  ...
  const submitFormData = (formData, resetForm) => {
    const shareToNewsFeedRequest = new Promise((resolve, reject) => {
      resolve(PostApi(API_Path.shareTipsAsPost, formData));
    });
      ...
      <Modal.Body>
        <Formik
          ...
          initialValues={{
            title: "",
            category: "",
            caption: props.tipDetails.description,
            ...
            <form className="row mb-0" onSubmit={runform.handleSubmit}>
              ...
              <div className="col-12 mb-3">
                <div className="modal-in-img">
                  {props.tipDetails?.image == "" ||
                  props.tipDetails.hasOwnProperty("image") == false ? (
                    <img
                      src={defaultLoopImg}
                      className="img-fluid w-100 news-img"
                      alt=""
                    />
                  ) : props.tipDetails.image?.[0]?.includes(".mp4") ||
                    props.tipDetails.image?.[0]?.includes(".mp3") ? (
                    <video width="420" height="235" controls>
                      <source src={props.tipDetails?.image} type="video/mp4" />
                    </video>
                  ) : (
                    <img
                      src={props.tipDetails?.image}
                      className="img-fluid w-100 news-img"
                      alt=""
                    />
                  )}
              ...
              <div className="col-12">
                <div className="d-flex align-items-center">
                  <div className="prayer-profile me-3">
                    {props.tipDetails?.user?.profile_img == "" ? (
                  ...
                  <div className="latest-news-small-header py-2">
                    <h4 className="d-flex align-items-center">
                      {props.tipDetails?.user?.fname +
                        " " +
                        props.tipDetails?.user?.lname}
                    </h4>
                    <span>
                      {props.tipDetails?.user?.fname +
                        " " +
...
export default SharedAsPostTip;
